...
public class ThreadedSearch<T> implements Searcher<T>, Runnable {
    ...
    public boolean search(T target, List<T> list) throws InterruptedException {
        ...
        for (int k = 0; k < numThreads; k++){
            int begin = (list.size()*k)/ numThreads;
            int end = (list.size()*(k+1))/ numThreads;
            ThreadedSearch checkSearch = new ThreadedSearch(target, list, begin, end, myAnswer);
            myThreads[k] = new Thread(checkSearch);
            myThreads[k].start();
        }
